	...
	$.html5Validate = (function() {	
		...
		return {
			...
			isRegex: function(ele, regex, params) {
				// 原始值和处理值
				var inputValue = ele.value, dealValue = inputValue, type = ele.getAttribute("type") + "" ,datatype=null;
				type = type.replace(/\W+$/, "");
			...
			isAllpass: function(elements, options) {
				...
				var self = this;
				var allpass = true
				  , remind = function(control, type, tag ,datatype) {
					var key = $(control).attr("data-key"), label = $("label[for='"+ control.id +"']"), text= '', placeholder, errormsg;
					...
					if ($(control).isVisible()) {
						if (type == "radio" || type == "checkbox") {
							...
							// 下拉值为空或文本框文本域等为空
							if(/^range$/i.test(datatype)){//如果是range则让提示右移，否则则会显示不全
								$(control).testRemind((tag == "empty" && text)? "请输入"+ text : OBJREG.prompt[tag] + text,{align:"left"});
							}else{
								$(control).testRemind((tag == "empty" && text)? "请输入"+ text : OBJREG.prompt[tag] + text);
							}
							control.focus();
				...
				elements.each(function(){
					var el = this, type = el.getAttribute("type"), tag = el.tagName.toLowerCase(), isRequired = $(this).hasProp("required") ,datatype=null;
					// type类型
					if (type) {
					...
					if (type == "radio" && isRequired) {
						...
						allpass? remind(el, type, "empty" ,datatype): remind(el, type, tag ,datatype);
						allpass = false;
		...
		html5Validate: function(callback, options) {
			...
			if ($.html5Validate.isSupport) {
				if (params.novalidate) {
					...
					elements.each(function() {
						var type = this.getAttribute("type") + "", typeReplaced = type.replace(/\W+$/, "") ,datatype=this.dataset.type;
